...
namespace GRBL_Plotter
{
    public partial class ControlSerialForm : Form        // Form can be loaded twice!!! COM1, COM2
    {
        ...
        /// *DAREAETAN*
        /// </summary>
        public event EventHandler<RemoteIREventArgs> RaiseRemoteIREvent;
        protected virtual void OnRaiseRemoteIREvent(RemoteIREventArgs e)
        { RaiseRemoteIREvent?.Invoke(this, e); }
                ...
                if (dataField.Length > 2)
                {
                    for (int i = 2; i < dataField.Length; i++)
                    {
                        ...
                        if (data.Length > 1)
                        {
                            ...
                            //*DARESTAN*
                            if (dataField[i].IndexOf("KR:") >= 0)            // Remote IR key vakue
                            {
                                machineState.KR = lblSrKR.Text = data[1];
                                OnRaiseRemoteIREvent(new RemoteIREventArgs(data[1].ToString()));
                                continue;
                            }
                        }
                    }
